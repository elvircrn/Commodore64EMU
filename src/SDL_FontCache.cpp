...
FC_Rect FC_MakeRect(float x, float y, float w, float h) {
	FC_Rect r = {static_cast<int>(x), static_cast<int>(y), static_cast<int>(w), static_cast<int>(h)};
	return r;
}
...
static_inline FC_Rect FC_RectUnion(FC_Rect A, FC_Rect B) {
	...
	{
		FC_Rect result = {static_cast<int>(x), static_cast<int>(y), static_cast<int>(FC_MAX(0, x2 - x)), static_cast<int>(FC_MAX(0, y2 - y))};
		return result;
	}
}
...
FC_Rect FC_DefaultRenderCallback(FC_Image *src,
																 FC_Rect *srcrect,
																 FC_Target *dest,
																 float x,
																 float y,
...
static Uint8 FC_GrowGlyphCache(FC_Font *font) {
...
#else
	SDL_Texture *new_level = SDL_CreateTexture(font->renderer,
	...
	set_color(new_level,
						font->default_color.r,
						font->default_color.g,
						font->default_color.b,
...
Uint8 FC_UploadGlyphCache(FC_Font *font, int cache_level, SDL_Surface *data_surface) {
	...
	if (!fc_has_render_target_support)
		...
		new_level = SDL_CreateTexture(renderer,
																	data_surface->format->format,
																	SDL_TEXTUREACCESS_TARGET,
																	data_surface->w,
																	data_surface->h);
		SDL_SetTextureBlendMode(new_level, SDL_BLENDMODE_BLEND);
...
static FC_GlyphData *FC_PackGlyphData(FC_Font *font,
																			Uint32 codepoint,
																			Uint16 width,
																			Uint16 maxWidth,
																			Uint16 maxHeight) {
...

	{
		...
		for (; *source_string != '\0'; source_string = U8_next(source_string)) {
			...
			packed = (FC_PackGlyphData(font,
																 FC_GetCodepointFromUTF8(&buff_ptr, 0),
																 glyph_surf->w,
			...
			if (packed || FC_PackGlyphData(font,
																		 FC_GetCodepointFromUTF8(&buff_ptr, 0),
																		 glyph_surf->w,
																		 surfaces[num_surfaces - 1]->w,
																		 surfaces[num_surfaces - 1]->h) != NULL) {
...
#else
Uint8 FC_LoadFont(FC_Font *font,
									FC_Target *renderer,
									const char *filename_ttf,
									Uint32 pointSize,
									SDL_Color color,
									int style)
#endif
...
static FC_Rect FC_RenderLeft(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale, const char *text) {
	...
	for (; *c != '\0'; c++) {
		...
		srcRect = glyph.rect;
#endif
		dstRect = fc_render_callback(FC_GetGlyphCacheLevel(font, glyph.cache_level),
																 &srcRect,
																 dest,
																 destX,
																 destY,
...
static void FC_RenderAlign(FC_Font *font,
													 FC_Target *dest,
													 float x,
													 float y,
													 int width,
													 ...
													 const char *text) {
	switch (align) {
	case FC_ALIGN_LEFT: FC_RenderLeft(font, dest, x, y, scale, text);
		break;
	case FC_ALIGN_CENTER: FC_RenderCenter(font, dest, x + width / 2, y, scale, text);
		break;
	case FC_ALIGN_RIGHT: FC_RenderRight(font, dest, x + width, y, scale, text);
		break;
	}
}
...
static void FC_DrawColumnFromBuffer(FC_Font *font,
																		FC_Target *dest,
																		FC_Rect box,
																		int *total_height,
																		FC_Scale scale,
...
FC_Rect FC_DrawBoxAlign(FC_Font *font,
												FC_Target *dest,
												FC_Rect box,
												FC_AlignEnum align,
...
FC_Rect FC_DrawBoxEffect(FC_Font *font,
												 FC_Target *dest,
												 FC_Rect box,
												 FC_Effect effect,
												 const char *formatted_text,
...
FC_Rect FC_DrawColumn(FC_Font *font, FC_Target *dest, float x, float y, Uint16 width, const char *formatted_text, ...) {
	FC_Rect box = {static_cast<int>(x), static_cast<int>(y), width, 0};
	int total_height;
...
FC_Rect FC_DrawColumnAlign(FC_Font *font,
													 FC_Target *dest,
													 float x,
													 float y,
													 Uint16 width,
	...
	switch (align) {
	case FC_ALIGN_CENTER: box.x -= width / 2;
		break;
	case FC_ALIGN_RIGHT: box.x -= width;
		break;
	default: break;
	}
...
FC_Rect FC_DrawColumnScale(FC_Font *font,
													 FC_Target *dest,
													 float x,
													 float y,
													 Uint16 width,
...
FC_Rect FC_DrawColumnColor(FC_Font *font,
													 FC_Target *dest,
													 float x,
													 float y,
													 Uint16 width,
...
FC_Rect FC_DrawColumnEffect(FC_Font *font,
														FC_Target *dest,
														float x,
														float y,
														Uint16 width,
...
static FC_Rect FC_RenderCenter(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale, const char *text) {
	...
	result =
			FC_RectUnion(FC_RenderLeft(font, dest, x - scale.x * FC_GetWidth(font, "%s", str) / 2.0f, y, scale, str), result);
...
static FC_Rect FC_RenderRight(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale, const char *text) {
	...
	for (c = str; *c != '\0';) {
		if (*c == '\n') {
			*c = '\0';
			result =
					FC_RectUnion(FC_RenderLeft(font, dest, x - scale.x * FC_GetWidth(font, "%s", str), y, scale, str), result);
			*c = '\n';
			c++;
			str = c;
			y += scale.y * font->height;
...
FC_Rect FC_DrawScale(FC_Font *font,
										 FC_Target *dest,
										 float x,
										 float y,
										 FC_Scale scale,
...
FC_Rect FC_DrawAlign(FC_Font *font,
										 FC_Target *dest,
										 float x,
										 float y,
										 FC_AlignEnum align,
	...
	FC_Rect result;
	switch (align) {
	case FC_ALIGN_LEFT: result = FC_RenderLeft(font, dest, x, y, FC_MakeScale(1, 1), fc_buffer);
		break;
	case FC_ALIGN_CENTER: result = FC_RenderCenter(font, dest, x, y, FC_MakeScale(1, 1), fc_buffer);
		break;
	case FC_ALIGN_RIGHT: result = FC_RenderRight(font, dest, x, y, FC_MakeScale(1, 1), fc_buffer);
		break;
	default: result = FC_MakeRect(x, y, 0, 0);
		break;
	}
...
FC_Rect FC_DrawColor(FC_Font *font,
										 FC_Target *dest,
										 float x,
										 float y,
										 SDL_Color color,
...
FC_Rect FC_DrawEffect(FC_Font *font,
											FC_Target *dest,
											float x,
											float y,
											FC_Effect effect,
	...
	FC_Rect result;
	switch (effect.alignment) {
	case FC_ALIGN_LEFT: result = FC_RenderLeft(font, dest, x, y, effect.scale, fc_buffer);
		break;
	case FC_ALIGN_CENTER: result = FC_RenderCenter(font, dest, x, y, effect.scale, fc_buffer);
		break;
	case FC_ALIGN_RIGHT: result = FC_RenderRight(font, dest, x, y, effect.scale, fc_buffer);
		break;
	default: result = FC_MakeRect(x, y, 0, 0);
		break;
	}
...
FC_Rect FC_GetBounds(FC_Font *font,
										 float x,
										 float y,
										 FC_AlignEnum align,
										 FC_Scale scale,
	...
	case FC_ALIGN_CENTER: result.x -= result.w / 2;
		break;
	case FC_ALIGN_RIGHT: result.x -= result.w;
		break;
	default: break;
	}
...
Uint16 FC_GetPositionFromOffset(FC_Font *font,
																float x,
																float y,
																int column_width,
																FC_AlignEnum align,
